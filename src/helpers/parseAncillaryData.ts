 ...
 */
export default function parseAncillaryData(
  ancillaryData: string
): Record<string, unknown> {
  // Some requesting contracts set the synthetic token address as ancillary data, so try to parse it first:
  if (ethers.utils.isAddress(ancillaryData)) return { address: ancillaryData };
  let ancillaryString;
  try {
    ancillaryString = ethers.utils.toUtf8String(ancillaryData);
  } catch (err) {
...
function parseAncillaryString(
  ancillaryString: string
): Record<string, unknown> {
  const ancillaryObject: Record<string, unknown> = {};
  const stringObject = Array.from(ancillaryString).map((character) => ({
    character,
    escape: false,
    skip: false,
  }));
...
function markEscapes(stringObject: CharObject[]) {
  stringObject.forEach((charObject, openIndex, stringObject) => {
    ...
    if (
      ...
      (isNextEnd(stringObject, openIndex, false) ||
        isNextChar(stringObject, openIndex, ",", false))
    )
...
function splitKeyValues(stringObject: CharObject[]): CharObject[][] {
  ...
  for (let startIndex = 0; startIndex < stringObject.length; startIndex++) {
    ...
    if (
      !skipWhitespace(charObject) ||
      (charObject.character === "," && !charObject.escape)
    )
      continue;
  ...
  return keyValues.map((keyValue: CharObject[]) =>
    keyValue.filter(removeDoubleQuotes)
  );
}
...
function parseKeyValue(keyValue: CharObject[]): [string, unknown] {
  ...
  let index =
    keyValue.findIndex(skipWhitespace) === -1
      ? keyValue.length
      : keyValue.findIndex(skipWhitespace);
  ...
  while (index < keyValue.length) {
    ...
    if (
      keyValue[index + skip].character === ":" &&
      !keyValue[index + skip].escape
    ) {
      index += 1 + skip;
  ...
  if (index === keyValue.length)
    throw new Error("Cannot parse key value pair: no column delimiter found!");
  ...
  while (index < keyValue.length) {
    ...
    if (
      ...
      throw new Error(
        "Cannot parse key value pair: multiple column delimiters found!"
      );

    value = value.concat(keyValue[index].character);
    index++;
  }
  if (!key || !value) throw new Error("Cannot parse key value pair!");
...
export function isNextChar(
  ...
  if (forward) {
    const nextCharIndex = stringObject
      .slice(start + 1)
      .findIndex(skipWhitespace);
    if (nextCharIndex === -1) {
      return false;
    } else {
      return (
...
function escapeQuotes(
  ...
  const nextCharFn = escapeValues
    ? function (stringObject: CharObject[], closeIndex: number): boolean {
        return (
          isNextEnd(stringObject, closeIndex) ||
          isNextChar(stringObject, closeIndex, ",")
        );
      }
    : function (stringObject: CharObject[], closeIndex: number): boolean {
  ...
  for (
    ...
    closeIndex < stringObject.length;
    closeIndex++
  ) {
    if (
      stringObject[closeIndex].character === '"' &&
      nextCharFn(stringObject, closeIndex)
    ) {
...
function skipWhitespace(charObject: CharObject): boolean {
  ...
  return (
    !whitespaceCharacters.includes(charObject.character) || charObject.escape
  );
}
